...
$mysql = new mysqli('localhost','root','1234','study');
...
if(isset($_POST['cookieName'])){
	...
	$result 	= $mysql->query("SELECT cookieHash, id FROM users WHERE login='".$login."'");

	$row = $result->fetch_array();
	if (md5($row ['cookieHash'].$login."069bcd26e") == $md5){
		$id = $row ['id'];
	}
	else{
		$id = 0;
	...
	echo '{"user_id":"'.$id.'", "user_name":"'.$login.'"}';
...
if(isset($_POST['auth']) and isset($_POST['userName']) and isset($_POST['pass'])){
	...
	if($row [0] > 0){
		$cookieHash = md5(uniqid(rand(),true));
   		...
   		$mysql->query("UPDATE users SET  cookieHash=\"".$cookieHash."\" WHERE id=".$row [0]);
   	}
